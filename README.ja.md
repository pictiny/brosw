...
URL を開くたびに、どの Chrome プロファイルで開くかをマウス位置のポップアップで選べる macOS メニューバーアプリ。

Terminal や Slack などのアプリでリンクを開いたとき、Chrome が仕事用・個人用どのプロファイルで開くかは予測しにくく、意図しない方で開いてしまいがちです。Brosw は macOS のデフォルトブラウザとして URL オープンを仲介し、プロファイル選択ポップアップ(以下、ピッカー)で開き先をその場で選ばせてくれます。

## 特徴

Brosw は軽量なアプリケーションです:

- **Chrome のプロファイル選択に特化** — 選ぶのはブラウザではなくプロファイル
...
URL を開くとマウスカーソルのそばにピッカーが現れ、各プロファイルが Chrome 本体と同じアバター・カラーで並びます。マウスでもキーボードでも選べます:
...
- Chrome プロファイルが 1 つしかない場合はピッカーを出さず即オープン
