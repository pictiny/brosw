# Brosw — Chrome プロファイル選択 URL ルーター 仕様書
...
Brosw は **macOS のデフォルトブラウザとして登録される仲介アプリ**である。URL オープンを横取りし、マウス位置にプロファイル選択ポップアップ(以下、**ピッカー**)を出して開き先の Chrome プロファイルをユーザーに選択させる。

### スコープ(v1)

- 選択単位は **プロファイルのみ**(既存ウィンドウ単位の選択はスコープ外)
- 開き先は**毎回ユーザーがピッカーで選択**する(URL パターンによる自動振り分けは持たない)
- 対象ブラウザは **Google Chrome のみ**
...
| 役割 | デフォルトブラウザとして http/https URL のオープンをすべて受け取り、選択されたプロファイルの Chrome で開く |
| 対象 OS | macOS 13+ |
...
3. ユーザーがプロファイルを選択 → 次のコマンドを `Process`(引数配列で渡す。シェル経由なし)で実行する
   ...
   ```
   /usr/bin/open -na "Google Chrome" --args --profile-directory="<dir>" "<url>"
   ```

   - Chrome 起動済みの場合、Chrome のシングルトン機構により既存プロセスへ転送され、該当プロファイルの最前面ウィンドウ(なければ新規ウィンドウ)で開く
4. パネルを閉じる

## 3. プロファイル列挙

- `~/Library/Application Support/Google/Chrome/Local State`(JSON)の `profile.info_cache` を読む
...
- アバター: Chrome 本体(`ProfileAttributesEntry::IsUsingGAIAPicture()`)と同じ優先順位で解決する
  ...
  3. どちらも解決できなければ、Chrome のプロファイルカラー(`default_avatar_fill_color` / `stroke_color`)の丸+イニシャル
...
| 設定 | 内容 |
|---|---|
| プロファイル非表示リスト | ピッカーの候補から除外するプロファイル(ディレクトリ名で記憶)。全件非表示にした場合は行き止まり防止のため全件表示にフォールバック。フィルタ後 1 件なら即オープン(§7)が適用される |
| メニューバーアイコン非表示 | ステータスアイコンを消す。URL 受信・ピッカー動作には影響しない |
| メールアドレス非表示 | ピッカーの各行からアカウントのメールアドレスを消す(設定画面のプロファイル一覧には表示したまま) |
| 並び順 | 「最近使った順」(既定)と「カスタム」を選択。カスタムは設定画面の ↑↓ ボタンで任意に並べ替え(ディレクトリ名の配列で記憶)。カスタム順に未登録の新規プロファイルは最近使った順で末尾に付く |
...
- Chrome 未インストール / 起動失敗 → アラートでエラー表示し、URL をクリップボードへコピー
- プロファイルが 1 つだけ → ピッカーを出さず即オープン
- プロファイルが 0 件(Local State が存在しない・パースできない)→ `--profile-directory` を付けずに Chrome を開き、Chrome 側の既定動作(初回起動フロー or 最後に使ったプロファイル)に任せる
- ピッカー表示中に再度 URL が届いた → 同一セッションへ追加(§4「複数 URL の扱い」)
- 対応スキームは `http` / `https` のみ(`mailto` 等は対象外)
- ローカル HTML(Finder で `.html` をダブルクリック)→ URL と同様にピッカーを表示し、選択したプロファイルの Chrome で `file://` URL として開く(デフォルトブラウザ化に伴い HTML ファイルの関連付けも Brosw に来るため)。ヘッダーにはホスト名の代わりにファイル名を表示する
...
└── Sources/Brosw/
    ...
    ├── ChromeProfiles.swift  # Local State パース・アバター解決
    ├── ChromeLauncher.swift  # open -na 実行
